...
CREATE TABLE IF NOT EXISTS `ACC_MONEY`(
    ...
    `STATUS` tinyint(4)  DEFAULT '1'            COMMENT '状态',
    PRIMARY KEY (`ID`),
)ENGINE=InnoDB DEFAULT CHARACTER=utf8 COMMENT='币种表';
...
CREATE TABLE IF NOT EXISTS `ACC_USER_ACCOUNT` (
    ...
    CONSTRAINT  ACCID  FOREIGN KEY(`ACC_ID`) REFERENCES ACC_ACCOUNT(`ID`)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
)ENGINE=InnoDB DEFAULT CHARACTER=utf8 COMMENT='用户记账表'
...
CREATE TABLE IF NOT EXISTS `ACC_ACCOUNT`(
    ...
    `DATETIME` date DEFAULT NOW()               COMMENT '消费时间',
    `STATUS` tinyint(4)  DEFAULT '1'            COMMENT '状态',
     PRIMARY KEY (`ID`),
     CONSTRAINT CATEGORY FOREIGN KEY(`CATEGORY`) REFERENCES ACC_CATEGORY(`ID`)
)ENGINE=InnoDB DEFAULT CHARACTER=utf8 COMMENT='支出表';
